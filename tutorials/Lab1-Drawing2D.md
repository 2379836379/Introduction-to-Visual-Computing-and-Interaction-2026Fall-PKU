...
这次 Lab 中，大家将会实现前 8 讲中介绍的几种重要的算法或思想：
 ...
 6. 第七讲 反走样 中的 Supersampling 算法
 
 7. 第六讲 曲线 中的 de Casteljau 算法
...
使用Poisson Editing算法（P47-P52）将飞机图片贴到未名湖上空，补全`void Inpainting(ImageRGB & output, ImageRGB const & inputBack, ImageRGB const & inputFront, const glm::ivec2 & offset)`函数，其中`inputFront`为飞机，`inputBack`为背景，`offset`为飞机对应部分相对于背景图像顶点的偏移。直接粘贴的方式效果如下：
